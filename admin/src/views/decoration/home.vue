<template>
    <div class="estate">
        <el-card shadow="never">
            <el-alert title="* 设置移动端商城首页导航区的菜单，支持选择跳转商城页面、或者填写自定义链接" type="info" :closable="false"></el-alert>
        </el-card>
            ...
            <el-table class="m-t-20" :data="pager.lists">
                <el-table-column label="菜单图标" prop="image">
                    <template #default="scope">
                        <div class="flex">
                            <el-image :src="scope.row.image" style="width: 70px; height: 70px"></el-image>
                        </div>
                    </template>
                </el-table-column>
                ...
                <el-table-column label="操作" width="150" fixed="right">
                    <template #default="scope">
                        ...
                        >
                            <template #trigger>
                                <el-button type="text">{{ scope.row.status ? '停用' : '启用' }}</el-button>
                            </template>
                            {{
