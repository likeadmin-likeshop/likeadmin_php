...
const eventResponse = {
	...
	success: ({ show, msg, data }: any): Promise<any> =>{
		if (show * 1) ElMessage({ type: 'success', message: msg });
		return data;
	},
	// 失败
	error: ({ show, msg }: any): Promise<any> =>{
		if (show * 1) ElMessage({ type: 'error', message: msg });
...
request.interceptors.response.use(
    (response) => {
        switch(response.data.code) {
            ...
            case -1: return eventResponse.redirect()
            case 2: return eventResponse.page(response.data)
        }
    },
    (error) => {
        console.log(error)
