  ...
  <div class="workbench">
    <div class="flex workbench-header">
      <el-card class="flex-1 !border-none" shadow="never">
        ...
        <div>
          <div class="m-b-20">当前版本：{{ workbenchData.version.version }}</div>
          <div>基于框架：vue3.x、ElementUI、MySQL</div>
    ...
    <div class="m-t-15 function">
      <el-card class="flex-1" shadow="never">
        ...
        <div class="nav-lists">
          <div v-for="item in workbenchData.menu" :key="item" class="nav-item flex-col m-t-10">
          ...
          <el-table :data="workbenchData.article" size="medium">
            <el-table-column label="排名" min-width="70" type="index">
              <template #default="scope">
                <div v-if="scope.$index == 0" class="icon" style="background: #f86056; color: #fff">
                  {{ scope.$index + 1 }}
                </div>
                <div v-if="scope.$index == 1" class="icon" style="background: #fc8d2e; color: #fff">
                  {{ scope.$index + 1 }}
                </div>
                <div v-if="scope.$index == 2" class="icon" style="background: #fcbc2e; color: #fff">
                  {{ scope.$index + 1 }}
                </div>
